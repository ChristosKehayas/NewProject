...
const paySettle = (debtInput, participantArray) => {
   ...
   // console.log("humans from paySettle",humans)
    //console.log("debt for paySettle",debtInput)
    let debt = debtInput;
    let stateC = 0;
    let stateD = 0;
    let StateDim = -1;
...
const paySettle2 = (participantArray) => {
...
const trackerCalc = (debtInput, participantArray) => {
    ...
    let permutatedInput2 = [...permutator(participantArray)].slice(0);
    // console.log(permutatedInput[0].slice(0))
    console.log(permutatedInput2[0].slice(0))
   
    let minL = paySettle(permutatedInput[0].slice(0), participantArray).length;
    //console.log(minL)
    let minL2 = paySettle2(permutatedInput2[0].slice(0)).length;
    //console.log(minL2)
    let maxL = Math.max(countPos(debtInput).poscounter, countPos(debtInput).negcounter);
    //console.log(maxL)
    let maxL2 = Math.max(countPos(permutatedInput2[0].slice(0).map(x=>x.debt)).poscounter, countPos(permutatedInput2[0].slice(0).map(x=>x.debt)).negcounter);
    //console.log(maxL2)
    let result = paySettle(permutatedInput[0].slice(0), participantArray);
    //console.log("result",result)
    let result2 = paySettle2(permutatedInput2[0].slice(0));
...
const finalCalc = () => {
    ...
    trackerCalc(debt, participantArray);
}
